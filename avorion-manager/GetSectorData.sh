...
for file in ${DIR}/${GALAXYNAME}/sectors/*v; do
  ...
  FACTIONINDEX=$(grep 'factionIndex' <<< $SECTORSTUFF | sed -e 's/.*factionIndex=//g')
  ...
  if [[ $FACTIONINDEX == 200* ]];
  then
    StartingPositionStateForm=$(grep -b -a -o -P 'state_form' "${DIR}/${GALAXYNAME}/factions/faction_${FACTIONINDEX}.dat" | sed 's/:.1*//' | head -n1 | awk '{SUM = $1 + 16 } END {print SUM}')
    StartingPositionName=$(grep -b -a -o -P 'name' "${DIR}/${GALAXYNAME}/factions/faction_${FACTIONINDEX}.dat" | sed 's/:.1*//' | head -n1 | awk '{SUM = $1 + 16 } END {print SUM}')
    Name=$(xxd -ps -l 50 -seek "${StartingPositionName}" "${DIR}/${GALAXYNAME}/factions/faction_${FACTIONINDEX}.dat" | xxd -r -p | head -n1)
    FactionName=$(xxd -ps -l 50 -seek ${StartingPositionStateForm} "${DIR}/${GALAXYNAME}/factions/faction_${FACTIONINDEX}.dat" | xxd -r -p | head -n1 | sed -e 's/static.*//g' -e "s/%s/"${Name}"/g" -e 's/[\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f\r\n]//g')
